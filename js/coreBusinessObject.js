...
(function(global) {
  ...
  async function generateCoreBusinessObjectForStepWithStrictPrompt(stepName, stageName, stepIndex, valueStreamJson, globalItGapJson, localItGapJson, rolePermissionJson) {
    const systemPrompt = `Role & Context: 我是一名需求分析专家。我已完成前期的全局 IT Gap 分析、局部节点 IT Gap 分析以及角色权限模型模拟。现在，我需要进行核心业务对象（Business Object）推演，为后续的全局 IT 架构设计提供底层数据模型支撑。
Input Data (上下文参考):
...
Output Format: 直接输出一个 JSON 对象，不要数组，不要 markdown 代码块。结构必须包含：
{
  ...
  function buildEntityCardHtml(entity, opts) {
    ...
    const usageBody = objectUsage ? formatCoreBusinessObjectField(objectUsage) : '<span class="problem-detail-core-business-object-empty">—</span>';
    const subcardsHtml = [
  ...
  function buildCoreBusinessObjectStepViewHtml(match) {
        ...
        ? roleGroups
            .map(([roleLabel, groupEntities]) => {
              const itemsHtml = groupEntities
                ...
                .filter(Boolean)
                .join('');
              const groupCountLabel = groupEntities.length > 0 ? `<span class="problem-detail-core-business-object-count-tag">${groupEntities.length}个对象</span>` : '';
              return `
    <div class="problem-detail-card problem-detail-card-core-business-design-group">
      <div class="problem-detail-card-header problem-detail-card-header-collapsed" tabindex="0" role="button" aria-expanded="false">
        <span class="problem-detail-card-header-title">${escapeHtml(roleLabel)}</span>${groupCountLabel}
        <span class="problem-detail-card-header-arrow">▾</span>
      </div>
      <div class="problem-detail-card-body" hidden>
    ...
    const totalCountLabel = entities.length > 0 ? `<span class="problem-detail-core-business-object-count-tag">${entities.length}个对象</span>` : '';
    const designCardHtml = `
    <div class="problem-detail-card problem-detail-card-core-business-design">
      <div class="problem-detail-card-header problem-detail-card-header-collapsed" tabindex="0" role="button" aria-expanded="false">
        <span class="problem-detail-card-header-title">核心业务对象设计</span>${totalCountLabel}
        <span class="problem-detail-card-header-arrow">▾</span>
      </div>
