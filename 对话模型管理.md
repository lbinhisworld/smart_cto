...
**说明**：意图提炼的 `taskId` 仅来自上述 task1~task6。ITGap 分析（大阶段 2）不通过 `taskId` 管理，其子步骤通过确认型提示块触发。

### 5.1 ITGap 阶段与子步骤

| 大阶段 | 子步骤（按顺序） | 管理方式 |
|--------|------------------|----------|
| ITGap分析 (currentMajorStage=2) | 端到端流程绘制、流程节点生成、交互节点设计、集成节点设计、逻辑节点设计 | `itGapCompletedStages` 数组 |
