...
> 最近更新：2026-03-19。与 [对话模型管理.md](./对话模型管理.md) 中任务列表（task1–task15）及意图类型、过程日志逻辑保持一致。  
...
### BMC 讨论模式（runBmcDiscussionTurn）

**触发**：用户点击 BMC 内容卡「讨论」并发送问题后，每次用户发言调用 `runBmcDiscussionTurn(baseBmcData, discussionHistory, userMessage)`。  
**user 内容结构**：四段拼接——【当前画布数据 base bmc data】+ 【bmc 绘制任务的历史讨论数据】+ 【用户当前输入】。  
**System Prompt**：BMC 讨论专用（角色、交互背景、分析框架、约束、输出规范）。

**输出规范**（讨论回复）：🧩 业务洞察（针对用户提问的直接回答）；🔗 BMC 关联影响（对 BMC 其他 1–2 个模块的联动影响）；🔍 引导性追问（1 个具有挑战性或启发性的问题）。
...
**用途**：基于客户基本信息、BMC、需求逻辑三个维度，生成业务核心价值流图；输出为单一 JSON，含设计逻辑说明（`logic_description`）与绘图数据（`vsm_data`）。  
**实际 Prompt 位置**：`main.js` 中的 `VALUE_STREAM_PROMPT`（并挂到 `window.VALUE_STREAM_PROMPT`）。

### System Prompt 要点（与 main.js 中常量一致）

- **角色设定**：资深业务架构师与需求分析专家，擅长通过价值流图（VSM）还原业务逻辑骨架，识别转化效率与协作断点。
- **输入数据说明**：`enterprise_info`（客户行业背景与规模）、`bmc_data`（商业模式画布）、`requirement_logic`（业务流转逻辑与痛点）。
- **业务逻辑深度诊断**：规避数据孤岛、神仙/贫血模块、循环依赖、状态孤儿；显性化影子流程；对齐颗粒度。
- **绘图指令与规范**：3–6 个核心阶段；每阶段 1–3 个原子任务；任务属性：任务名称（动宾）、执行角色、预估耗时/频率；逻辑连接用箭头体现前后序依赖。
- **输出格式**：**必须仅输出一个合法的 JSON 字符串**，不得包含任何 Markdown 格式标识、前言或后缀。
...
- 前端解析：整段返回内容做 `JSON.parse`，取 `parsed.vsm_data` 作为价值流图数据（存为 `valueStream`），取 `parsed.logic_description` 作为设计逻辑（存为 `valueStreamLogicText`）；工作区「价值流设计逻辑」卡片展示后者，价值流图根据前者渲染。
...
- **确认后动画**：用户点击任务通知「确认」后，聊天区先显示「正在进行全局 ITGap 分析」动画（parsing 块），再发起大模型调用。
- **任务上下文不入聊天区**：Task8 的任务上下文内容卡片（taskContextBlock）仅写入沟通历史过程日志，不在聊天区展示。
...
- **流程**：用户点击「即将生成每个环节的 ITGap 分析 session」→「确认」后，系统**先**向 task9 过程日志推送两条独立「上下文」块（价值流图 json、全局 ITGap 分析 json），**不向聊天区推送**；再根据端到端流程生成各环节 session（不调用大模型）；用户点击 session 列表的「确认」或「继续」后，仅显示「正在分析环节【X】」并逐环节调用大模型，不弹完成确认。
- **确认后直接开始**：用户点击 session 列表的「确认」后，直接开始逐环节分析，无需二次确认。
- **继续按钮**：session 列表卡片在「确认」右侧有「继续」按钮，存在未完成分析时可用，点击后继续执行剩余环节的分析。
- **每环节完成**：每环节完成后推送局部 ITGap 卡片（confirmed:false，带确认/重做/修正/讨论），不自动进下一环节；分析结果同步更新工作区对应 session 卡片（待分析→已分析✅），并写入 Task9 任务过程日志。
- **全部环节确认后**：聊天区先推送**压缩确认块**「我即将开始对局部 ITGap 分析做上下文压缩，便于后续环节的处理。」+ 确认；用户点击确认后系统按环节依次调用大模型压缩各环节分析 JSON，每完成一环向 task9 时间线推送「压缩」+ 环节名 + 耗时/token + 压缩后 JSON；**全部压缩完成后**再推送**任务完成确认块**「是否确认局部 ITGap 分析任务已经完成？」+ 确认；用户点击该确认后推送「任务完成」、推进状态并切换当前任务为角色与权限模型推演（task10）。
- **过程日志**：时间线卡片在标签右侧显示环节名称；两条独立「上下文」块（价值流图 json、全局 ITGap 分析 json）仅入过程日志、不入聊天区；session 确认条目标签旁备注「ITGap 分析 session 计划」；压缩条目标签「压缩」+ 环节名；输入卡片 token 为 prompt_tokens、耗时为 0，输出卡片 token 为 completion_tokens、耗时为模型返回；局部 ITGap 卡片支持**重做**（可清除该环节结果并重新分析）。
